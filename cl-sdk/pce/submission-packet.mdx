---
title: "Generating PCE Submission Packets for Carriers"
sidebarTitle: "Submission Packet"
description: "Generate carrier-ready PCE submission packets, run quality gate checks, and work through the review checklist before handing off to a licensed professional."
---
...
if (report.qualityGateStatus === "failed") {
  // Keep the case in draft and surface blocking issues to the user.
  console.log(report.blockingIssues);
}
```

<ResponseField name="qualityGateStatus" type='"passed" | "passed_with_warnings" | "failed"'>
  Overall readiness signal. `"passed"` means no blocking issues and no missing-info questions. `"passed_with_warnings"` means warnings exist but the case can proceed to review. `"failed"` means blocking issues or missing info must be resolved first.
</ResponseField>

<ResponseField name="blockingIssues" type="CaseValidationIssue[]">
  Subset of `validationIssues` with `severity === "blocking"`. Each issue includes an `itemId`, `fieldPath`, and human-readable `message`.
</ResponseField>

<ResponseField name="warningIssues" type="CaseValidationIssue[]">
  Issues with `severity === "warning"` that a reviewer should assess but that do not block submission.
</ResponseField>

<ResponseField name="missingInfoCount" type="number">
  Number of unresolved missing-info questions at report time.
</ResponseField>

<ResponseField name="citationCoverage" type="number">
  Fraction of change items that have at least one verified citation (0–1). Low coverage suggests the evidence retrieval step found limited supporting text.
</ResponseField>
