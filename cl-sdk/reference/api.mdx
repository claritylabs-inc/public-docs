...
<ParamField body="sourceSpans" type="SourceSpan[]">
  For PDF input, parse the document first and pass the resulting source spans. The extractor does not render PDFs. Pre-parsed Docling input can supply source evidence through its document.
</ParamField>
