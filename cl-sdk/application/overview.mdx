---
title: "Insurance Application Processing Pipeline"
sidebarTitle: "Overview"
description: "Process insurance application PDFs end-to-end with an agentic pipeline that extracts fields, auto-fills from context, batches questions, and maps a final PDF."
---
...
<ParamField path="applicationStore" type="ApplicationStore" required>
  Persistence store for application state. The pipeline reads and writes state here at each phase.
</ParamField>

<ParamField path="documentStore" type="DocumentStore" required>
  Store used by the `lookup-filler` agent to search policy records during document search fill.
</ParamField>

<ParamField path="memoryStore" type="MemoryStore" required>
  Conversation memory store used to maintain context across the reply loop.
</ParamField>
