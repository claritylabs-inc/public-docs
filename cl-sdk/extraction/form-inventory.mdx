---
title: "Form Structure and Source Tree Organization"
...
<ResponseField name="limits" type="OperationalCoverageTerm[]">
  Structured limit terms for this coverage line. Each term carries a `termKind` (e.g. `"each_claim"`, `"aggregate"`, `"retention"`) and a `value` string.
</ResponseField>
...
function collectEndorsements(nodes: DocumentSourceNode[]): DocumentSourceNode[] {
  return nodes.flatMap((node) => {
    const children = node.children ? collectEndorsements(node.children) : [];
    return node.kind === "endorsement" ? [node, ...children] : children;
  });
}

const result = await extractor.extract(pdfBase64, "doc-123", { sourceSpans });
const endorsements = collectEndorsements(result.sourceTree ?? []);
