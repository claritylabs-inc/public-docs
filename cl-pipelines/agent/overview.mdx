...
`runAgent` is a convenience wrapper around `runPipeline` that drives a multi-turn LLM conversation. Each turn in the conversation is a single pipeline phase named `"turn"`. The agent saves message history between turns and resumes from the last checkpoint after a crash. Work performed after that checkpoint, including API calls and tool actions, may run again on retry.
