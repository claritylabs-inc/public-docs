...
2. Calls `await store.hydrate()` to load IndexedDB and run any pending migrations.
3. If `flushOnHydrate` is `true`, calls `await store.flushPendingMutations()`.
4. Notifies all subscribers, causing `useSyncStatus().hydrated` to become `true`.
