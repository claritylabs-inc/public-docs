...
<ResponseField name="createScopeKey(scope)" type="string">
  Derives a composite scope key string in the format `"appId:env:userId:orgId"`. Omitted `environment`, `userId`, and `orgId` default to `default`, `anonymous`, and `none`.
...
| `useHydratedValue(localValue, serverValue)` | `<T>(local: T \| undefined, server: T \| undefined) => T \| undefined` | Returns the supplied server value; otherwise returns local data after hydration, or `undefined` before hydration. |
