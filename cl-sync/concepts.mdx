...
CL Sync calls `createScopeKey(scope)` to produce a composite string key in the form `"appId:env:userId:orgId"`. Records are scoped by app, environment, user, and organization within a database named by app and environment. Switching scope does not delete previously stored data.
